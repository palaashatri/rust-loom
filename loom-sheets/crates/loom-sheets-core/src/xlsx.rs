...
pub fn export_xlsx_sheets(sheets: &[Sheet]) -> Result<Vec<u8>, String> {
    ...
    for (index, sheet) in sheets.iter().enumerate() {
        ...
        if has_drawing {
            ...
            let drawing_relationship = "<drawing r:id=\"rId1\"/>".to_string();
...
fn render_chart_xml(sheet: &Sheet, chart: &SheetChart) -> String {
    ...
    let cat_range = format!("{name}!${cat_column}$2:${cat_column}${end_row}");
    let val_range = format!("{name}!${val_column}$2:${val_column}${end_row}");
