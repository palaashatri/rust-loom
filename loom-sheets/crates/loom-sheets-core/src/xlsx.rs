...
pub fn import_xlsx_sheets(xlsx_bytes: &[u8]) -> Result<XlsxImport, String> {
    let archive = PackageArchive::from_bytes(xlsx_bytes)
        .map_err(|e| format!("unreadable xlsx archive: {e}"))?;
    let warnings = detect_import_warnings(&archive)?;
    Ok(XlsxImport {
...
fn detect_import_warnings(archive: &PackageArchive) -> Result<Vec<XlsxImportWarning>, String> {
...
fn parse_xml_nodes(path: &str, xml: &str) -> Result<Vec<XmlNode>, String> {
...
fn parse_relationships(path: &str, bytes: &[u8]) -> Result<Vec<OoxmlRelationship>, String> {
    let xml = std::str::from_utf8(bytes).map_err(|_| format!("{path} is not valid UTF-8"))?;
    let nodes = parse_xml_nodes(path, xml)?;
    Ok(nodes
        .into_iter()
        .filter(|node| node.namespace == PACKAGE_RELATIONSHIPS_NS && node.name == "Relationship")
        .filter_map(|node| {
