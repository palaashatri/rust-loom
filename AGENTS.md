...
**Sheets source-size status (2026-09-25):** The Sheets maintenance split now puts CSV interop, command dispatch, close handling, file completion ordering, and worker-failure recovery in named modules. `loom-sheets-app/src/main.rs` is 106,978 bytes against its 108,783-byte legacy ceiling; `ui/app.slint` is 39,629 bytes against its 39,732-byte ceiling. The new Rust and Slint modules remain below their source limits. The six generated renderer captures under `loom-sheets/docs/qa-renderer/` have a narrowly scoped provenance rule. The latest repo-wide code-structure audit reports 17 findings outside the active Sheets source scope and none in files changed by REC-02 stage 3; do not edit locked applications or unrelated shared modules to clear them. The asset audit passes.
...
**REC-02 Stage 4B — bounded read-only inspection (2026-09-26) — NEEDS_REVIEW.** `RecoveryInspectionCursor` exposes a selected checkpoint and streams one verified journal record at a time; it enforces finite checkpoint, metadata, directory-entry, journal, record-count, and line-size ceilings, and stops permanently after an error. Exact journal stats become available at EOF. The collector remains available for callers with bounded workloads. Inspection does not repair, compact, or prune. Verification under WSL/Linux: focused inspector tests passed 17/17; full `loom-production` tests passed 57/57; all-target Clippy with `-D warnings` and formatting passed. Independent review found a remaining check/open race: checkpoint path sizes are preflighted, then the existing loader reopens those paths; a non-cooperating path replacer could bypass the package read ceiling. The caller-held shared recovery lock protects cooperating writers only. Do not use this API as a hard boundary against non-cooperating path replacement until loading is bounded through opened handles. Windows/macOS behavior and Sheets startup integration are also unverified. REC-02 remains OPEN.

From the repository root in WSL: cargo test --manifest-path loom-core/Cargo.toml -p loom-production --locked recovery_journal_bounds_tests:: -- --nocapture passed 7/7; cargo test --manifest-path loom-core/Cargo.toml -p loom-production --locked passed 40/40 (0 doc tests); cargo clippy --manifest-path loom-core/Cargo.toml -p loom-production --locked --all-targets -- -D warnings passed; cargo fmt --manifest-path loom-core/Cargo.toml --all -- --check passed; git diff --check exited 0 with only Git LF-to-CRLF notices. loom-core/crates/loom-production/src/lib.rs is 61,980 bytes, below the 65,536-byte limit. Remaining Stage 4 work includes policy integration, startup inspection that preserves over-limit stores, automatic checkpoint cadence, UI Retry/Save As and edit admission, failure injection, and the approved performance measurements.
...
**Remaining REC-02 gaps:** The bounded append and read-only inspection APIs exist, but the approved write policy is not enforced in the Sheets worker and startup does not yet replay through the read-only cursor into an unpublished candidate. The inspection loader still has a check/open race against non-cooperating path replacement; Windows/macOS behavior is unverified. Automatic checkpoint cadence and journal/record admission limits are not integrated. Recovery admission does not pause after a failed or lagging append. Disk-full, interruption-at-each-publication-step, two-instance, ordinary journal cap tests, and the unchanged million-cell recovery-latency/retained-byte measurements remain open. Do not mark REC-02 fixed or Sheets accepted.
