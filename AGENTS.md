...
**Sheets source-size status (2026-09-25):** The Sheets maintenance split now puts CSV interop and command dispatch in named modules. Current sizes are `loom-sheets-core/src/lib.rs` 192,859 bytes (below the 199,529-byte legacy ceiling), `loom-sheets-app/src/actions.rs` 61,219 bytes, and `loom-sheets-app/src/command_dispatch.rs` 5,192 bytes (both below the 65,536-byte Rust limit). The six generated renderer captures under `loom-sheets/docs/qa-renderer/` now have a narrowly scoped provenance rule. The fresh repo-wide code-structure audit still reports 18 failures, all outside the active Sheets scope; do not edit locked applications to clear them. The asset audit passes.
...
**Next milestone:** asynchronous CSV/XLSX export. Keep the overall Sheets gate blocked.
...
**Next milestone:** move CSV/XLSX export behind the workbook worker's FIFO revision barrier and separate file-completion queue. Keep the overall Sheets gate blocked.
