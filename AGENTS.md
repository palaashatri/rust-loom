...
**Sheets data-safety status (2026-09-26):** REC-02 remains OPEN after stages 1–3, bounded append (Stage 4A), bounded startup inspection/replay (Stage 4B), and partial write-side integration (Stage 4C1). Sheets writes versioned raw cell-edit batches after a complete native package checkpoint in an isolated recovery directory; startup validates and replays them. Legacy full-package data is migrated only after the matching versioned checkpoint is published and read back; cleanup uses the shared `.checkpoint.lock`, a manifest comparison, a retryable receipt, and a completed marker. Without a receipt, any known or unsupported legacy entry beside versioned recovery causes startup to fail before journal repair, preserving both stores when their ordering is unknown. Temporary startup imports and canceled replacements do not migrate. Stage 4C1 routes bounded cell batches through `append_bounded`; an oversized batch or journal-limit refusal falls back to a complete workbook checkpoint without splitting the batch or advancing its sequence. That ordinary checkpoint path does not yet run the approved package, retained-storage, or temporary-peak preflight, so the write-side storage caps are not enforced. The worker mutates its candidate before journaling and still evaluates/publishes it after a recovery error; edit admission is not paused. Batches can remain queued behind prior workbook calculation, so recovery can lag visible edits. Startup inventory and legacy migration enforce approved read/storage ceilings, and checkpoint reconciliation retains two generations. Production checkpoint cadence, truthful Retry/Save As, admission pause, disk-full/interruption coverage, and measured durability latency remain open. The export/close milestone retains consumed worker input errors and blocks Save, Export, and Close until a full worker replacement succeeds; New/Open preserve the existing dirty-workbook decision. CODE-23 detects known XLSX losses and asks before replacing a workbook in both the file picker and startup `--open` paths. The shared command dispatcher blocks native-menu and palette actions while the decision is open, including actions that were queued just before the warning appeared. Tests cover actual defined names and confirm that an empty `<definedNames/>` container does not warn, plus external links, conditional formatting, validation rules, PivotTables, frozen panes, custom row/column sizes, extra charts, missing drawing/media parts, a supported no-warning workbook, Cancel, Continue, startup recovery preservation, blocked command dispatch, and Escape after Tab navigation. A native screenshot captured the visible menu row and CODE-23 import-warning dialog on 2026-09-24, but native button, focus, menu-popup, and close interactions remain unverified; do not treat a still image as interaction proof. The XLSX warning is not proof of complete Excel compatibility. Read CODE-23 and REC-02 before changing these paths. When changing XLSX import, warn before replacing the open workbook; Cancel must leave both the workbook and recovery data untouched.
...
**Stage 4C1 checkpoint:** Code and verification above are committed as `d7561b58fa46c68f69bd9f3009512ac093796914` and pushed to `origin/main`. On 2026-09-26, `git ls-remote origin refs/heads/main` returned the same commit; the local worktree was clean. Stage 4C2 was started on 2026-09-26 and is being checkpointed as incomplete below.

**REC-02 Stage 4C2 — ordinary write-side capacity preflight (2026-09-26) — INCOMPLETE; NEEDS_REVIEW.** The draft adds a lock-scoped checkpoint-and-compaction path, append/checkpoint budget projections, ordinary append/checkpoint preflight callbacks in `CellEditRecovery`, and refusal regressions that preserve journal/checkpoint bytes and sequence, including a torn journal tail. It preserves the scanner's existing directory charge and adds a 64 KiB reserve for each new directory entry. The RED tests ran before the implementation and failed for the expected missing-refusal behavior: `ordinary_checkpoint_capacity_refusal_preserves_checkpoint_journal_and_sequence` reported no capacity refusal, and `ordinary_append_capacity_refusal_preserves_torn_journal_and_sequence` reported no aggregate-limit refusal. The production package suite passed 64 tests (`.work/sheets-acceptance-2026-09-26/rec02-stage4c2-production-tests.log`). The latest focused Sheets command, `cargo test --manifest-path loom-sheets/Cargo.toml -p loom-sheets-app --bin loom-sheets 'ordinary_(checkpoint|append)_capacity_refusal' --locked --offline -- --nocapture --test-threads=1`, failed to compile with six E0277 errors because inventory and overflow helpers returned `String` where the callbacks require `ProductionError`. A narrow error mapping was then added in `cell_edit_recovery.rs`, but no build or test has been run after that edit. Both core and Sheets formatting checks passed before that last error-mapping edit. No focused GREEN test, Clippy, workspace suite, governance/assets/source audit, or independent review has been completed for this draft. Treat cap enforcement, append refusal preservation, checkpoint publication, and compaction behavior as unverified. Windows/macOS behavior remains unverified. This checkpoint is pushed only to preserve the current draft; it does not complete Stage 4C2 or REC-02.

From the repository root in WSL: cargo test --manifest-path loom-core/Cargo.toml -p loom-production --locked recovery_journal_bounds_tests:: -- --nocapture passed 7/7; cargo test --manifest-path loom-core/Cargo.toml -p loom-production --locked passed 40/40 (0 doc tests); cargo clippy --manifest-path loom-core/Cargo.toml -p loom-production --locked --all-targets -- -D warnings passed; cargo fmt --manifest-path loom-core/Cargo.toml --all -- --check passed; git diff --check exited 0 with only Git LF-to-CRLF notices. loom-core/crates/loom-production/src/lib.rs is 63,714 bytes, below the 65,536-byte limit. Remaining Stage 4 work includes ordinary checkpoint package/storage preflight, production checkpoint cadence, truthful Retry/Save As and edit admission, failure injection, and the approved performance measurements.
