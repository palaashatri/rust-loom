...
**Sheets performance status (2026-09-25):** Formula-bar commits send one changed cell to a bounded worker and show “Calculating…” immediately. Picker-selected and startup Open parsing run on a background loader. Native Save and Save As use a worker barrier at revision N: the worker builds one package, atomically writes it, checkpoints the same bytes, and returns a separate FIFO completion. Save completion updates the baseline only for its document generation and leaves newer revisions dirty. The UI now says newer edits remain unsaved and recovery may still be catching up. A crash before the later recovery batch completes can still lose N+1; that recovery-freshness gap remains open under PERF-01 and REC-02. CSV/XLSX exports and many non-cell full-workbook copies remain synchronous or clone on the UI thread. The last fixed million-cell run (2026-09-24, Intel Core i3-2350M) measured 2.56 seconds for evaluation, 12.72 seconds for package creation, and 43.05 seconds for recovery-journal writing; callback, visible-frame, and current recovery-freshness measurements remain open. Do not mark PERF-01 or Sheets accepted until those paths, native scrolling, the reviewed app memory limit, accessibility, and interoperability have passing evidence. Use the running Linux window and native screenshot tool only while the desktop is unlocked; a renderer picture is not live-window proof.

**Sheets source-size status (2026-09-25):** The 2026-09-24 maintenance split moved CLI parsing, headless rendering, XLSX import flow, app tests, Slint dialogs, and XLSX responsibilities into named modules. A fresh audit on this `main` checkout contradicts the earlier clean-sheet-size statement: `loom-sheets-core/src/lib.rs` is 205,155 bytes against its 199,529-byte legacy ceiling, and `loom-sheets-app/src/actions.rs` is 66,378 bytes against the 65,536-byte Rust limit. The repo-wide audit reported 20 failures, including 18 outside the active Sheets scope. Do not claim the Sheets source-size gate is clean. The separate asset audit also flags six existing generated renderer captures under `loom-sheets/docs/qa-renderer/` as missing provenance rules.
...
**Remaining Open gaps:** The loader cannot be cooperatively stopped during parsing and its JoinHandle detaches on shutdown. Native picker/dialog interaction, current live-window screenshot, complete startup wiring plus recovery journey, and full input-to-visible-frame timing remain unverified. Native Save/Save As now have focused asynchronous ordering coverage, but a crash can still lose N+1 before its queued recovery batch completes; the UI warns that newer edits are unsaved and recovery may still be catching up. CSV/XLSX export, non-cell full-workbook copies, recovery latency/storage bounds, native million-cell scrolling, reviewed app memory limit, visual/accessibility coverage, and broader interoperability remain open. The Sheets acceptance gate stays blocked.
...
**Next milestone:** clear the two active Sheets code-size findings and register verified project-generated renderer captures, then continue with asynchronous CSV/XLSX export. Keep the overall Sheets gate blocked.
