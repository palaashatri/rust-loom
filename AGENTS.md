...
|---:|---|---|---|---|
| 1 | Sheets | ACCEPTANCE_BLOCKED | IN_PROGRESS | Native menu-popup interaction and warning-dialog focus/actions; Open worker shutdown and live timing; slow and unbounded recovery storage; synchronous Save/Save As/CSV/XLSX and full-replacement copies; full visual/accessibility coverage; broader interoperability; native million-cell scroll; reviewed app memory budget |
...
**Remaining performance gate:** Formula-bar cell commits now send a cell delta to the background worker and preserve newer revisions/drafts. Large-workbook evaluation and recovery can still take tens of seconds, while non-cell replacements and normal file operations have not been moved off the UI thread. See PERF-01; this card does not prove native frame timing or the peak-memory limit.
...
**Remaining Open gaps:** The active loader cannot be cooperatively stopped during parsing and its JoinHandle detaches on shutdown. No native picker/dialog interaction, current live-window screenshot, complete startup wiring plus recovery journey, or full input-to-visible-frame timing was verified in this run. The review confirmed the formula-draft P1 and held-candidate P2 are fixed; the XLSX-held-candidate cleanup now has a focused test. Save/Save As/CSV/XLSX, full-workbook copies, recovery latency/storage bounds, native million-cell scrolling, and the app memory budget remain open. Keep PERF-01 and the Sheets acceptance gate blocked.

### REC-02 — Keep unsaved recovery safe and bounded

**P1 · Sheets persistence and storage · OPEN.** Each changed workbook can add another complete workbook package to the local recovery journal. The journal is compacted only by the explicit Save checkpoint, so an unsaved session has no automatic disk-space bound. The append path also rereads and validates the growing journal for every new record. On this personal computer, a long editing session must not be allowed to quietly consume all available storage. A concrete, unapproved format and threshold proposal is recorded in [the REC-02 recovery design review](AGENTS.md#source-loom-sheets-docs-qa-reports-rec-02-recovery-design-review-md); implementation must wait for owner review of that proposal.
...
**For a tiny coding model:** (1) Keep the current full-package journal readable. Do not change its file format without the reviewed recovery design. (2) Make the newest edit durable before waiting for slow recalculation or package building. (3) Add a recovery limit and a full checkpoint that covers a known edit sequence. Write and verify the new checkpoint first; only then delete journal entries covered by it. Keep newer entries. (4) Replay edits from the checkpoint in order, and preserve formulas, blank-cell deletes, active tab, added/deleted tabs, and embedded image bytes. (5) If any edit cannot be written or replayed, show a recovery error; do not report it as saved. (6) Test restart after every write/rename, a damaged journal, two app instances, a full disk, and a long unsaved session. Record both the maximum recovery storage and how much recent work a forced crash can lose.

**Done when:** A long unsaved session stays within a reviewed storage bound, the newest accepted edit is quickly recoverable, restart reconstructs the exact workbook, and failure tests prove an old valid checkpoint survives every interrupted compaction step. The concrete journal format and thresholds still require design review before code changes.
<!-- CURRENT TRUTH END -->
