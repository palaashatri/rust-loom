...
|---:|---|---|---|---|
| 1 | Sheets | IN_PROGRESS | IN_PROGRESS | UI-01 native visual check, full interoperability/performance checks, and broader accessibility coverage remain |
...
- Portable audit evidence bundle, including the code and UI/UX reports, historical audit evidence, the 2026-09-22 Sheets acceptance report/UI-14 evidence, the expanded 2026-09-23 native screenshot/interaction report and screenshots (including UI-06 status/error captures), the UI-01 renderer preview, and the UI-17 and UI-25 repair reports/screenshots: [loom-bootstrap/audit-evidence-2026-09-14-and-22.zip](loom-bootstrap/audit-evidence-2026-09-14-and-22.zip) (SHA-256 `e2558a6408e82871864e9fb33869a6a93759a6391e4376af2adea10373372679`).
- Native Sheets self-audit: [.work/sheets-acceptance-2026-09-23/REPORT.md](.work/sheets-acceptance-2026-09-23/REPORT.md) records live `gnome-screenshot` captures plus native keyboard, AT-SPI, and Orca checks at 1024×720. UI-06 status/error captures are also checked into `loom-sheets/docs/qa-native/`. The 1440×900 native capture remains unavailable on this 1366×728 desktop; other themes/viewports use existing renderer evidence. The 2026-09-23 UI-01 follow-up adds a tested local menu for non-macOS; its native visual capture is pending because Cinnamon's full-screen screensaver is active.
...
**Repair result (2026-09-23):** Sheets now draws File, Edit, View, Table, and Help menus inside every non-macOS window. macOS keeps its native global menu. The local menu is projected from the installed menu model, uses the same command IDs and enabled/checked state, and dispatches through the existing guarded menu action sink. Help > Keyboard Shortcuts opens the existing command palette. Unsupported items and empty menus are hidden. The native `NativeMenuBar` installer only calls the OS menu API under `target_os = "macos"`; generating Linux DBusMenu layout data does not install or display a Linux global menu.

**Verification:** The non-macOS accessibility-tree test finds File, Edit, View, and Table in the application window. Local menu projection/dispatch tests pass; the full Sheets app suite passes 115 tests; and the native app build succeeds. The live Linux screenshot attempt is not valid evidence: Cinnamon's full-screen screensaver was active above the app window, so `gnome-screenshot` captured the desktop overlay. No screensaver, lock, or display setting was changed. Repeat the native visual check after the desktop session is unlocked.

**Evidence:** Source in `loom-sheets/crates/loom-sheets-app/ui/app.slint`, `src/local_menu.rs`, and `src/main.rs`. Test: `non_macos_window_exposes_a_local_application_menu_bar`. Detailed commands and the blocked native capture are in `.work/sheets-acceptance-2026-09-23/REPORT.md` and the portable audit archive. Older screenshots `sheets/17-current-native-start.png`, `22-current-native-overflow.png`, and `21-current-native-palette.png` preserve the original failure state; they do not show this repair.
