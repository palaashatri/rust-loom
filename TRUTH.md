...
**Source evidence (2026-09-24):** `extract_xlsx_sheets` builds a Loom `Sheet` from cells, styles, alignments, charts, shapes, and images. The `Sheet` model has no place for workbook defined names, conditional formatting, data validation, or external links, and the importer does not inspect those OOXML parts. `open_workbook_from_picker` replaces the current workbook after import and shows only the generic `Imported …; use Save As for a Loom workbook` status. A later Loom save cannot keep features that were discarded during import. This was confirmed by source inspection; a dedicated feature-loss fixture and native warning test have not yet been run.

**For a tiny coding model:** (1) Keep a short list of XLSX features Loom imports and features it drops. (2) Before replacing the current workbook, check whether the XLSX has dropped features. If it does, show their names and ask the user to continue or cancel. (3) Cancel must leave the current workbook and its recovery data unchanged. (4) Do not call the import fully compatible unless Loom really keeps every feature it claims to support. (5) Add one small XLSX fixture per detected feature and test the warning, Cancel, and Continue paths. Preserve unsupported XML only if the workbook model and save path can safely round-trip it; never keep unknown XML by guessing where it belongs.

**Done when:** Users see a clear warning before Loom discards unsupported workbook features, cancellation leaves the open workbook unchanged, supported imports do not show a false warning, and the documented feature list matches tested behavior. Broader Excel and LibreOffice feature coverage remains a separate interoperability gate.
