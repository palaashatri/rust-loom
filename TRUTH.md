...
document blocks (`writer_document_digest`), workbook cells (`Workbook::integrity_digest`),
deck slides/elements/notes (`PresentationDocument::integrity_digest`), raster pixels plus layer metadata
(`RgbaImage::pixel_digest`, `PhotoDocument::metadata_digest`), composition layers/keyframes
...
Vision table extraction into dense editable grids with per-cell confidence (`OcrTableCell`, `grid_from_ocr_table`) in Sheets;
accessibility alt-text coverage auditing with clean-fraction summaries (`AccessibilityFinding`, `audit_accessibility`) in Present;
segmentation-mask conversion to layer masks with iterative region counting (`mask_bytes_to_rgba`, `count_mask_regions`) in Photo;
template dependency manifests with deduplicated font/media/composition keys (`TemplateDependency`, `collect_template_dependencies`) in Motion;
caption-lane entries derived from SRT cues with overlap rejection and playhead lookup (`CaptionEntry`, `captions_from_cues`) in Video;
RMS/peak loudness summaries with make-up gain suggestions (`LoudnessSummary`, `analyze_loudness`) in Studio; and
persisted-queue schema migration from single-destination V1 payloads to multi-destination V2
(`migrate_queue_payload_v1_to_v2`, `migrate_queue_batch_v1_to_v2`) in Encode. All 11 monorepo
workspaces pass unit, integration, formatting, Clippy, UI audit, contract audit, and offline test
gates with 635 passing automated tests.

## Vision-to-application bridges

The shared `loom_vision_core::transfer` vocabulary (`OcrTextBlock`, `MaskSpan`) is
consumed by Writer (editable paragraphs with block-index provenance), Sheets (dense
grids with per-cell confidence), and Photo (layer masks with region counting), so
Vision results become ordinary editable data rather than flat overlays.
...
- A crash-tolerant recovery journal (`loom_history::RecoveryJournal`) with
  ...
  remain appendable after truncation.
- Uniform FNV-1a integrity digests over user-authored state in every
  application core, enabling save/reload verification and tamper detection.
...
- **Sheets:** multi-sheet workbook management (`add_sheet`, `remove_sheet`, `rename_sheet`),
  ...
  functions via deterministic bisection (`goal_seek_bisection`), proleptic Gregorian date arithmetic via civil-day
  conversion (`is_leap_year`, `days_in_month`, `add_days`, `days_between`), CSV dialect sniffing across comma, semicolon,
...
- **Photo:** raster decode, pixel buffers, layers, arbitrary-angle image rotation with exact 90-degree fast paths and
  ...
  XML attribute escaping (`PhotoDocument::to_openraster_stack_xml`), segmentation-mask conversion to layer masks with
  iterative region counting (`mask_bytes_to_rgba`, `count_mask_regions`), pixel and metadata integrity digests
  (`RgbaImage::pixel_digest`, `PhotoDocument::metadata_digest`), a round paint-brush engine with hardness falloff and spaced dab
...
- **Motion:** layer/keyframe models, damped harmonic spring overshoot keyframe arrival physics
  ...
  synthesis (`HandheldConfig`, `calculate_handheld_offset`), layer parenting hierarchy resolution composing position,
  scale, and rotation down ancestor chains (`SampledTransform`, `compose_parented_transform`, `resolve_world_transform`),
  an SVG path `d`-attribute subset parser for M/L/Z commands with relative-coordinate resolution
  ...
  (`RenderQueueEntry`, `RenderOutputKind`), template dependency manifests with deduplicated font/media/composition keys
  (`TemplateDependency`, `collect_template_dependencies`), composition integrity digests over layers and keyframes
  (`CompositionDocument::integrity_digest`),
...
- **Video:** track/clip models, J/K/L shuttle transport state with a signed speed ladder
  ...
  (`EdlRecord`, `build_edl_records`, `write_edl`), caption-lane entries derived from SRT cues with overlap rejection and playhead lookup
  (`CaptionEntry`, `captions_from_cues`), timeline state digests over tracks, clips, and markers
  (`VideoProject::timeline_digest`), proxy media linkage registry with staleness detection for relink workflows
...
- **Studio:** track/region models, metronome click-track generation with accented downbeats and decaying bursts
  ...
  (`StemEntry`, `StemsExportPlan`), RMS/peak loudness summaries with make-up gain suggestions
  (`LoudnessSummary`, `analyze_loudness`), session integrity digests over tracks and regions
  (`StudioSession::session_digest`), pitch-class chord detection across seven qualities with confidence scoring
...
- **Encode:** editable FFmpeg queue, batch output collision resolution plus cross-platform filename sanitization
  (`resolve_output_collisions`, `sanitize_output_filename`), exponential-backoff retry policies for transient encode
  ...
  persisted-queue schema migration from single-destination V1 payloads to multi-destination V2
  (`migrate_queue_payload_v1_to_v2`, `migrate_queue_batch_v1_to_v2`),
  queue state digests over jobs and lifecycle transitions (`EncodeQueue::queue_digest`),
