...
**P1 · Shared · NEEDS_REVIEW · Recovery-loss paths corrected; Linux/Windows CI rerun pending.** The payload and its checksum are one thing. Replacing just one half makes the saved copy unreadable.
...
**Verification:** `cargo fmt --manifest-path loom-core/Cargo.toml --all -- --check`, `cargo metadata --manifest-path loom-core/Cargo.toml --format-version 1 --locked`, `cargo metadata --manifest-path loom-sheets/Cargo.toml --format-version 1 --locked`, the governance unit tests, governance audit, asset audit, UI-foundation audit, and `git diff --check` passed locally. Focused test command: `cargo test --manifest-path loom-core/Cargo.toml -p loom-production --locked`; Linux and Windows CI results are pending. The earlier workflow run passed Ubuntu recovery tests but exposed the Windows test assumption and missing Sheets lockfile entries. The general governance job also reports the existing code-size ratchet on four unchanged application files; that unrelated limit remains open. A read-only expert review found no remaining recovery blocker by inspection.

**Open:** Await the focused Linux and Windows CI rerun. Keep this card NEEDS_REVIEW until both pass.
