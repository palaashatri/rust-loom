...
**P2 · Sheets warning dialog · OPEN.** The one-feature native warning capture leaves a large empty block between the warning copy and its buttons. The dialog has a fixed 456 px height and stretches the warning text area to fill the spare space. Long loss lists still need a readable warning and buttons that stay visible.
...
**For a tiny coding model:** (1) Edit only `loom-sheets/crates/loom-sheets-app/ui/xlsx_import_warning.slint`. (2) Let the box grow from its title, warning text, buttons, and padding instead of always using 456 px. (3) Set a maximum height based on the available window. If the warning is too long, scroll the warning text inside the box while keeping both buttons visible. (4) Keep the existing Escape-to-Cancel focus scope unchanged. (5) Render one short warning and one long warning at 1024×720 and 2× text size. Check that no text or button is clipped and that the short warning has no oversized blank block.

**Evidence:** The native 1018×744 capture is `loom-sheets/docs/qa-native/ui23-xlsx-import-warning-live-linux.png`; SHA-256 `d2736f235b6eb463b49a93d89d086ff960460613eab6d294193417150a2f38d9`. It shows the large empty area for the one-feature case. No layout change has been implemented yet.
