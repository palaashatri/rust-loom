...
pub fn decode_audio_waveform_with_cancel(
    ...
    let stdout = child
        .stdout
        .take()
        .ok_or_else(|| "FFmpeg waveform stdout was not captured".to_string())?;
    let stderr = child
        .stderr
        .take()
        .ok_or_else(|| "FFmpeg waveform stderr was not captured".to_string())?;
    let stdout_reader = std::thread::spawn(move || {
        let mut bytes = Vec::new();
        let result = std::io::Read::read_to_end(&mut BufReader::new(stdout), &mut bytes);
        result.map(|_| bytes).map_err(|error| error.to_string())
    ...
    let stderr = stderr_reader
        ...
        .map_err(|error| format!("read FFmpeg waveform errors: {error}"))?;
    if !status.success() {
        return Err(stderr);
    }
    ...
    Ok(compute_waveform_peaks(&samples, target_bins))
}
