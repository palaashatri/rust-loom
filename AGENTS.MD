...
**Sheets visual note (2026-09-24):** The live warning screenshot `loom-sheets/docs/qa-native/ui23-xlsx-import-warning-live-linux.png` shows the one-warning dialog leaving a large blank area because its height is fixed at 456 px. UI-28 records the finding. To fix it, size the dialog to its text and buttons, cap it to the window, and scroll only the warning text when a long list does not fit. Keep both actions visible and keep Escape mapped to Cancel. Check a short and long warning at 1024×720 and 2× text size before closing the card.
