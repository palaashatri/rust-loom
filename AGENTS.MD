...
**Sheets XLSX import status (2026-09-24):** Standard Excel workbooks use ZIP DEFLATE compression. The shared package reader used by Sheets previously accepted only uncompressed ZIP entries and rejected a normal Calc-generated `.xlsx` with `unsupported compression method 8`. It now reads stored and DEFLATE entries, including entries whose sizes follow the compressed data in a ZIP data descriptor. It checks uncompressed size limits before inflation, caps output, and checks descriptor values and CRC. `loom-package` has regression fixtures for both ZIP layouts. A Calc-generated workbook now imports four sheets, formulas, a styled cell, a chart, and a shape label; custom row/column sizes are still named by the existing import warning. The 29 package tests, 266 Sheets workspace tests, and package all-target Clippy pass. Full-workspace Clippy and the production build were run before this shared-reader change; do not describe them as rerun. See CODE-25 for simple repair steps and remaining Excel/interoperability limits.

**Sheets data-safety status (2026-09-24):** REC-02 remains open: recovery currently saves another whole workbook after an edit and only clears old copies on explicit Save, so there is no automatic storage limit. CODE-23 detects known XLSX losses and asks before replacing a workbook in both the file picker and startup `--open` paths. The shared command dispatcher blocks native-menu and palette actions while the decision is open, including actions that were queued just before the warning appeared. Tests cover actual defined names and confirm that an empty `<definedNames/>` container does not warn, plus external links, conditional formatting, validation rules, PivotTables, frozen panes, custom row/column sizes, extra charts, missing drawing/media parts, a supported no-warning workbook, Cancel, Continue, startup recovery preservation, blocked command dispatch, and Escape after Tab navigation. The latest workspace run passes 266 tests, including the XLSX style-relationship and Save Changes Escape regressions. The preceding full-workspace Clippy and production build were run before CODE-25; fresh ZIP-package tests and Clippy are recorded above. Live native import-warning review is pending because Cinnamon is locked. The XLSX warning is not proof of complete Excel compatibility. Read CODE-23 and REC-02 before changing these paths. The recovery file format needs the reviewed design first. When changing XLSX import, warn before replacing the open workbook; Cancel must leave both the workbook and recovery data untouched.
