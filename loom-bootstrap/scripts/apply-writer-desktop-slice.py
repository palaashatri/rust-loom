#!/usr/bin/env python3
"""Apply the first Loom desktop-authenticity slice to Writer.

This one-shot migration is intentionally deterministic: every replacement must
match exactly once, otherwise it exits before writing partially modified files.
"""
...
def update_cargo() -> None:
    path = Path("loom-writer/crates/loom-writer-app/Cargo.toml")
    text = path.read_text()
    text = replace_once(
        text,
        'loom-writer-core = { path = "../loom-writer-core" }\n',
        'loom-writer-core = { path = "../loom-writer-core" }\n'
        'loom-desktop = { path = "../../../loom-core/crates/loom-desktop" }\n',
        "writer desktop dependency",
    )
    path.write_text(text)


def update_ui() -> None:
    path = Path("loom-writer/crates/loom-writer-app/ui/app.slint")
    text = path.read_text()
    text = replace_once(
        text,
        "    callback save-doc;\n    callback export-pdf;",
    ...
    text = replace_once(
        ...
        '            context: "Local document package";',
        "writer header context",
    )
    text = replace_once(
        text,
        '            ToolButton { icon: "save"; text: "Save"; clicked => { root.save-doc(); } }\n',
        '            ToolButton { icon: "save"; text: "Save"; clicked => { root.save-doc(); } }\n'
        '            ToolButton { icon: "save"; text: "Save As"; clicked => { root.save-as-doc(); } }\n',
        "save-as toolbar",
    )
    text = replace_once(
        text,
...
fn save_file(path: &Path, doc: &WriterDocument) -> Result<(), String> {
    ...
    std::fs::write(path, bytes)
        ...
        "path-aware file functions",
    )
    text = replace_once(
        text,
        "    SaveDoc,\n    ExportPdf,",
        "    SaveDoc,\n    SaveAsDoc,\n    ExportPdf,",
        "save-as palette enum",
    )
    text = replace_once(
        text,
        '''        PaletteCommand {
...
''',
    ...
    text = text.replace("Some(p) => load_file(p)?", "Some(p) => load_file(Path::new(p))?")
    text = replace_once(
        text,
    ...
    let state = Rc::new(GuiState {
        ...
        "initialize native dialog state",
    )
    text = replace_once(
        text,
        '''        app.on_new_doc(move || {
